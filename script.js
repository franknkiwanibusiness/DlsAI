...
// --- 2. UI UTILITIES (LUXE REDESIGN + SMART MATH + UNLOCK PATCH) ---
...
window.openVisionChat = async (reportText) => {
    ...
    // 1. SMART EXTRACTION: Priority on AI's Final Total
    // Extracts the value inside **$XX.XX** at the end of the AI report
    const finalPriceMatch = reportText.match(/\*\*?\$(\d+[\d,.]*)\*\*?/);
    ...
    if (auth.currentUser) {
        try {
            ...
            await set(ref(db, `users/${auth.currentUser.uid}/history/${scanId}`), {
                ...
                report: reportText,
                timestamp: Date.now(),
                imageUrl: scanPreview.src
            });
        } catch (e) { console.error("Auto-save failed:", e); }
    }

    // 5. TYPEWRITER (With Auto-Scroll)
    if (output) {
        output.innerText = ""; 
        let i = 0;
...
const resetScannerUI = (closeModal = true) => {
    ...
    // 4. Unlock Scroll & Clear Classes
    document.body.style.overflow = '';             
    document.body.classList.remove('loading-lock'); 
    document.body.classList.remove('modal-open');   

    // 5. Reset Internal View
...
window.closeResults = () => {
    ...
    if (resModal) {
        ...
        setTimeout(() => {
            ...
            resetScannerUI(true); // Full cleanup and unlock
        }, 300);
    } else {
        resetScannerUI(true);
    }
 ...
 * Automatically manages body scroll based on modal visibility
 */
const syncBodyScroll = () => {
    // 1. Find anything that looks like a modal or overlay
    const allPopups = document.querySelectorAll('.active, [id*="Modal"], [id*="Overlay"]');
    
    // 2. Check if any of them are actually visible right now
    const isAnythingOpen = Array.from(allPopups).some(el => {
        const style = window.getComputedStyle(el);
        // It's open if it has the 'active' class OR is set to display flex/block
        return el.classList.contains('active') || (style.display !== 'none' && style.visibility !== 'hidden');
    });

    // 3. Apply the lock or unlock
    if (isAnythingOpen) {
        document.body.style.overflow = 'hidden';
        document.body.classList.add('loading-lock');
    } else {
...
// Watch for any clicks on the page to trigger a check
document.addEventListener('click', () => setTimeout(syncBodyScroll, 100));

// Watch for any automatic HTML changes (like Firebase closing a modal)
const uiObserver = new MutationObserver(syncBodyScroll);
...
// Run once on startup to ensure we start fresh
syncBodyScroll();
