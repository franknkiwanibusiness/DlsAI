...
function getSiteUrl(req) {
    // 1. Use Origin header (sent by browser)
    if (req.headers.origin) return req.headers.origin;
    // 2. Construct from request
    const protocol = req.headers['x-forwarded-proto'] || req.protocol || 'https';
    const host = req.get('host');
    if (host) return `${protocol}://${host}`;
    // 3. Ultimate fallback (should never happen in normal use)
    return 'https://dlsvalue.site';
}
...
router.post('/initiate', async (req, res) => {
    try {
        ...
        // ── FIXED: Get site URL from request, not env ──────────────────────
...
<div style="max-width:520px;margin:0 auto;padding:40px 20px;">
  ...
  <div style="text-align:center;font-size:11px;color:rgba(139,138,168,0.35);">© 2026 ${domain} · Automated security notice</div>
        ...
        if (emailFn) {
            ...
            console.warn('[account-transfer] No sendEmail function found');
        }
...
router.post('/accept', async (req, res) => {
    try {
        ...
        updates[`pendingTransfers/${fromUid}`] = null;
